...
bool ImGui::Checkbox( const char* label, bool* v )
{
	...
	if ( g.LogEnabled )
		LogRenderedText( &total_bb.Min, *v ? XS( "[x]" ) : XS( "[ ]" ) );
	if ( label_size.x > 0.0f )
...
bool ImGui::RadioButton( const char* label, bool active )
{
	...
	if ( g.LogEnabled )
		LogRenderedText( &total_bb.Min, active ? XS( "(x)" ) : XS( "( )" ) );
	if ( label_size.x > 0.0f )
		RenderText( ImVec2( check_bb.Max.x + style.ItemInnerSpacing.x, check_bb.Min.y + style.FramePadding.y ), label );
...
void ImGui::ProgressBar( float fraction, const ImVec2& size_arg, const char* overlay )
{
	...
	if ( !overlay )
	{
		ImFormatString( overlay_buf, IM_ARRAYSIZE( overlay_buf ), XS( "%.0f%%" ), fraction * 100 + 0.01f );
		overlay = overlay_buf;
	}
...
static const ImGuiDataTypeInfo GDataTypeInfo[] =
{
	...
	{ sizeof( ImU64 ),            XS( XS( "%llu" ) ), XS( XS( "%llu" ) )  },
#endif
...
bool ImGui::DragScalarN( const char* label, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, float power )
{
	...
	for ( int i = 0; i < components; i++ )
	{
		...
		value_changed |= DragScalar( XS( "" ), data_type, p_data, v_speed, p_min, p_max, format, power );
		PopID();
		PopItemWidth();
...
bool ImGui::SliderScalarN( const char* label, ImGuiDataType data_type, void* v, int components, const void* v_min, const void* v_max, const char* format, float power )
{
	...
	for ( int i = 0; i < components; i++ )
	{
		...
		if ( i > 0 )
			SameLine( 0, g.Style.ItemInnerSpacing.x );
		value_changed |= SliderScalar( XS( "" ), data_type, v, v_min, v_max, format, power );
		PopID();
		PopItemWidth();
...
bool ImGui::InputScalar( const char* label, ImGuiDataType data_type, void* p_data, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags )
{
	...
	if ( p_step != NULL )
	{
		...
		if ( ButtonEx( XS( "-" ), ImVec2( button_size, button_size ), button_flags ) )
		{
...
bool ImGui::InputScalarN( const char* label, ImGuiDataType data_type, void* p_data, int components, const void* p_step, const void* p_step_fast, const char* format, ImGuiInputTextFlags flags )
{
	...
	for ( int i = 0; i < components; i++ )
	{
		...
		if ( i > 0 )
			SameLine( 0, g.Style.ItemInnerSpacing.x );
		value_changed |= InputScalar( XS( "" ), data_type, p_data, p_step, p_step_fast, format, flags );
		PopID();
		PopItemWidth();
		p_data = ( void* )( ( char* )p_data + type_size );
	}
...
bool ImGui::InputFloat( const char* label, float* v, float step, float step_fast, int decimal_precision, ImGuiInputTextFlags flags )
{
	char format[ 16 ] = XS( "%f" );
	if ( decimal_precision >= 0 )
		ImFormatString( format, IM_ARRAYSIZE( format ), XS( "%%.%df" ), decimal_precision );
...
bool ImGui::InputInt( const char* label, int* v, int step, int step_fast, ImGuiInputTextFlags flags )
{
	...
	const char* format = ( flags & ImGuiInputTextFlags_CharsHexadecimal ) ? XS( "%08X" ) : XS( "%d" );
	return InputScalar( label, ImGuiDataType_S32, ( void* )v, ( void* )( step > 0 ? &step : NULL ), ( void* )( step_fast > 0 ? &step_fast : NULL ), format, flags );
}

bool ImGui::InputInt2( const char* label, int v[ 2 ], ImGuiInputTextFlags flags )
{
	return InputScalarN( label, ImGuiDataType_S32, v, 2, NULL, NULL, XS( "%d" ), flags );
}

bool ImGui::InputInt3( const char* label, int v[ 3 ], ImGuiInputTextFlags flags )
{
	return InputScalarN( label, ImGuiDataType_S32, v, 3, NULL, NULL, XS( "%d" ), flags );
}

bool ImGui::InputInt4( const char* label, int v[ 4 ], ImGuiInputTextFlags flags )
{
	return InputScalarN( label, ImGuiDataType_S32, v, 4, NULL, NULL, XS( "%d" ), flags );
}
...
bool ImGui::ColorEdit4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags )
{
	...
	if ( ( flags & ( ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV ) ) != 0 && ( flags & ImGuiColorEditFlags_NoInputs ) == 0 )
	{
		...
		const bool hide_prefix = ( w_item_one <= CalcTextSize( ( flags & ImGuiColorEditFlags_Float ) ? XS( "M:0.000" ) : XS( "M:000" ) ).x );
		...
		for ( int n = 0; n < components; n++ )
		{
			...
			if ( !( flags & ImGuiColorEditFlags_NoOptions ) )
				OpenPopupOnItemClick( XS( "context" ) );
		}
	}
	else if ( ( flags & ImGuiColorEditFlags_DisplayHex ) != 0 && ( flags & ImGuiColorEditFlags_NoInputs ) == 0 )
	{
		...
		char buf[ 64 ];
		if ( alpha )
			ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "#%02X%02X%02X%02X" ), ImClamp( i[ 0 ], 0, 255 ), ImClamp( i[ 1 ], 0, 255 ), ImClamp( i[ 2 ], 0, 255 ), ImClamp( i[ 3 ], 0, 255 ) );
		else
			ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "#%02X%02X%02X" ), ImClamp( i[ 0 ], 0, 255 ), ImClamp( i[ 1 ], 0, 255 ), ImClamp( i[ 2 ], 0, 255 ) );
		SetNextItemWidth( w_inputs );
		if ( InputText( XS( "##Text" ), buf, IM_ARRAYSIZE( buf ), ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase ) )
		{
			value_changed = true;
			char* p = buf;
			...
			if ( alpha )
				...
				sscanf( p, XS( "%02X%02X%02X" ), ( unsigned int* )&i[ 0 ], ( unsigned int* )&i[ 1 ], ( unsigned int* )&i[ 2 ] );
		}
		if ( !( flags & ImGuiColorEditFlags_NoOptions ) )
			OpenPopupOnItemClick( XS( "context" ) );
	}
	...
	if ( !( flags & ImGuiColorEditFlags_NoSmallPreview ) )
	{
		...
		if ( ColorButton( XS( "##ColorButton" ), col_v4, flags ) )
		{
		...
		if ( BeginPopup( XS( "picker" ) ) )
		{
			...
			value_changed |= ColorPicker4( XS( "##picker" ), col, picker_flags, &g.ColorPickerRef.x );
			EndPopup();
		}
	}
...
bool ImGui::ColorPicker4( const char* label, float col[ 4 ], ImGuiColorEditFlags flags, const float* ref_col )
{
	...
	if ( flags & ImGuiColorEditFlags_PickerHueWheel )
	{
		...
		InvisibleButton( XS( "hsv" ), ImVec2( sv_picker_size + style.ItemInnerSpacing.x + bars_width, sv_picker_size ) );
		if ( IsItemActive() )
		{
	...
	{
		// SV rectangle logic
		InvisibleButton( XS( "sv" ), ImVec2( sv_picker_size, sv_picker_size ) );
		if ( IsItemActive() )
		{
		...
		InvisibleButton( XS( "hue" ), ImVec2( bars_width, sv_picker_size ) );
		if ( IsItemActive() )
		{
	...
	if ( alpha_bar )
	{
		...
		InvisibleButton( XS( "alpha" ), ImVec2( bars_width, sv_picker_size ) );
		if ( IsItemActive() )
		{
...
void ImGui::ColorTooltip( const char* text, const float* col, ImGuiColorEditFlags flags )
{
	...
	{
		if ( flags & ImGuiColorEditFlags_NoAlpha )
			Text( XS( "H: %.3f, S: %.3f, V: %.3f" ), col[ 0 ], col[ 1 ], col[ 2 ] );
		else
			Text( XS( "H: %.3f, S: %.3f, V: %.3f, A: %.3f" ), col[ 0 ], col[ 1 ], col[ 2 ], col[ 3 ] );
	}
	EndTooltip();
}
...
void ImGui::ColorEditOptionsPopup( const float* col, ImGuiColorEditFlags flags )
{
	...
	if ( ( !allow_opt_inputs && !allow_opt_datatype ) || !BeginPopup( XS( "context" ) ) )
		return;
	ImGuiContext& g = *GImGui;
	ImGuiColorEditFlags opts = g.ColorEditOptions;
	if ( allow_opt_inputs )
	{
		if ( RadioButton( XS( "RGB" ), ( opts & ImGuiColorEditFlags_DisplayRGB ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DisplayMask ) | ImGuiColorEditFlags_DisplayRGB;
		if ( RadioButton( XS( "HSV" ), ( opts & ImGuiColorEditFlags_DisplayHSV ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DisplayMask ) | ImGuiColorEditFlags_DisplayHSV;
		if ( RadioButton( XS( "Hex" ), ( opts & ImGuiColorEditFlags_DisplayHex ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DisplayMask ) | ImGuiColorEditFlags_DisplayHex;
	}
	if ( allow_opt_datatype )
	{
		if ( allow_opt_inputs ) Separator();
		if ( RadioButton( XS( "0..255" ), ( opts & ImGuiColorEditFlags_Uint8 ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DataTypeMask ) | ImGuiColorEditFlags_Uint8;
		if ( RadioButton( XS( "0.00..1.00" ), ( opts & ImGuiColorEditFlags_Float ) != 0 ) ) opts = ( opts & ~ImGuiColorEditFlags__DataTypeMask ) | ImGuiColorEditFlags_Float;
	}

	if ( allow_opt_inputs || allow_opt_datatype )
		Separator();
	if ( Button( XS( "Copy as.." ), ImVec2( -1, 0 ) ) )
		OpenPopup( XS( "Copy" ) );
	if ( BeginPopup( XS( "Copy" ) ) )
	{
		int cr = IM_F32_TO_INT8_SAT( col[ 0 ] ), cg = IM_F32_TO_INT8_SAT( col[ 1 ] ), cb = IM_F32_TO_INT8_SAT( col[ 2 ] ), ca = ( flags & ImGuiColorEditFlags_NoAlpha ) ? 255 : IM_F32_TO_INT8_SAT( col[ 3 ] );
		char buf[ 64 ];
		ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "(%.3ff, %.3ff, %.3ff, %.3ff)" ), col[ 0 ], col[ 1 ], col[ 2 ], ( flags & ImGuiColorEditFlags_NoAlpha ) ? 1.0f : col[ 3 ] );
		if ( Selectable( buf ) )
			SetClipboardText( buf );
		ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "(%d,%d,%d,%d)" ), cr, cg, cb, ca );
		if ( Selectable( buf ) )
			SetClipboardText( buf );
		ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "#%02X%02X%02X" ), cr, cg, cb );
		if ( Selectable( buf ) )
			SetClipboardText( buf );
		if ( !( flags & ImGuiColorEditFlags_NoAlpha ) )
		{
			ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "#%02X%02X%02X%02X" ), cr, cg, cb, ca );
			if ( Selectable( buf ) )
				SetClipboardText( buf );
		}
		EndPopup();
...
void ImGui::ColorPickerOptionsPopup( const float* ref_col, ImGuiColorEditFlags flags )
{
	...
	if ( ( !allow_opt_picker && !allow_opt_alpha_bar ) || !BeginPopup( XS( "context" ) ) )
		return;
	ImGuiContext& g = *GImGui;
	if ( allow_opt_picker )
	{
		...
		for ( int picker_type = 0; picker_type < 2; picker_type++ )
		{
			...
			if ( Selectable( XS( "##selectable" ), false, 0, picker_size ) ) // By default, Selectable() is closing popup
				g.ColorEditOptions = ( g.ColorEditOptions & ~ImGuiColorEditFlags__PickerMask ) | ( picker_flags & ImGuiColorEditFlags__PickerMask );
			SetCursorScreenPos( backup_pos );
			ImVec4 dummy_ref_col;
			memcpy( &dummy_ref_col, ref_col, sizeof( float ) * ( ( picker_flags & ImGuiColorEditFlags_NoAlpha ) ? 3 : 4 ) );
			ColorPicker4( XS( "##dummypicker" ), &dummy_ref_col.x, picker_flags );
			PopID();
		}
		PopItemWidth();
	}
	if ( allow_opt_alpha_bar )
	{
		...
		CheckboxFlags( XS( "Alpha Bar" ), ( unsigned int* )&g.ColorEditOptions, ImGuiColorEditFlags_AlphaBar );
	}
	EndPopup();
}
...
void ImGui::TreePush( const char* str_id )
{
	...
	Indent();
	window->DC.TreeDepth++;
	PushID( str_id ? str_id : XS( "#TreePush" ) );
}
...
void ImGui::TreePush( const void* ptr_id )
{
	...
	Indent();
	window->DC.TreeDepth++;
	PushID( ptr_id ? ptr_id : ( const void* )XS( "#TreePush" ) );
}
	...
	if ( values_count >= values_count_min )
	{
		...
		if ( hovered && inner_bb.Contains( g.IO.MousePos ) )
		{
			...
			if ( plot_type == ImGuiPlotType_Lines )
				SetTooltip( XS( "%d: %8.4g\n%d: %8.4g" ), v_idx, v0, v_idx + 1, v1 );
			else if ( plot_type == ImGuiPlotType_Histogram )
				SetTooltip( XS( "%d: %8.4g" ), v_idx, v0 );
			v_hovered = v_idx;
		}
...
{
	Text( XS( "%s: %s" ), prefix, ( b ? XS( "true" ) : XS( "false" ) ) );
}

void ImGui::Value( const char* prefix, int v )
{
	Text( XS( "%s: %d" ), prefix, v );
}
...
void ImGui::Value( const char* prefix, float v, const char* float_format )
{
	...
	{
		char fmt[ 64 ];
		ImFormatString( fmt, IM_ARRAYSIZE( fmt ), XS( "%%s: %s" ), float_format );
		Text( fmt, prefix, v );
	}
	else
	{
		Text( XS( "%s: %.3f" ), prefix, v );
	}
}
...
bool ImGui::BeginMainMenuBar()
{
	...
	bool is_open = Begin( XS( "##MainMenuBar" ), NULL, window_flags ) && BeginMenuBar();
	PopStyleVar( 2 );
...
void    ImGui::EndTabBar()
{
	...
	if ( tab_bar == NULL )
	{
		IM_ASSERT_USER_ERROR( tab_bar != NULL, XS( "Mismatched BeginTabBar()/EndTabBar()!" ) );
		return;
	}
...
bool    ImGui::BeginTabItem( const char* label, bool* p_open, ImGuiTabItemFlags flags )
{
	...
	if ( tab_bar == NULL )
	{
		IM_ASSERT_USER_ERROR( tab_bar, XS( "BeginTabItem() Needs to be called between BeginTabBar() and EndTabBar()!" ) );
		return false;
	}
...
void    ImGui::EndTabItem()
{
	...
	if ( tab_bar == NULL )
	{
		IM_ASSERT( tab_bar != NULL && XS( "Needs to be called between BeginTabBar() and EndTabBar()!" ) );
		return;
	}
...
bool    ImGui::TabItemEx( ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags )
{
	...
	if ( g.HoveredId == id && !held && g.HoveredIdNotActiveTimer > 0.50f && IsItemHovered() )
		if ( !( tab_bar->Flags & ImGuiTabBarFlags_NoTooltip ) )
			SetTooltip( XS( "%.*s" ), ( int )( FindRenderedTextEnd( label ) - label ), label );
...
bool ImGui::TabItemLabelAndCloseButton( ImDrawList* draw_list, const ImRect& bb, ImGuiTabItemFlags flags, ImVec2 frame_padding, const char* label, ImGuiID tab_id, ImGuiID close_button_id )
{
	...
	const char* TAB_UNSAVED_MARKER = XS( "*" );
...
ImGuiID ImGui::GetColumnsID( const char* str_id, int columns_count )
{
	...
	ImGuiID id = window->GetID( str_id ? str_id : XS( "columns" ) );
	PopID();
