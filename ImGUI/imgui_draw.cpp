...
{
	IM_ASSERT( _Current == 0 && _Count <= 1 && XS( "Nested channel splitting is not supported. Please use separate instances of ImDrawListSplitter." ) );
...
ImFontAtlas::~ImFontAtlas()
{
	IM_ASSERT( !Locked && XS( "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!" ) );
	Clear();
}
...
ImFont* ImFontAtlas::AddFontDefault( const ImFontConfig* font_cfg_template )
{
	...
	if ( font_cfg.Name[ 0 ] == '\0' )
		ImFormatString( font_cfg.Name, IM_ARRAYSIZE( font_cfg.Name ), XS( "ProggyClean.ttf, %dpx" ), ( int )font_cfg.SizePixels );
	font_cfg.EllipsisChar = ( ImWchar )0x0085;

	const ImWchar* glyph_ranges = font_cfg.GlyphRanges != NULL ? font_cfg.GlyphRanges : GetGlyphRangesDefault();
	ImFont* font = AddFontFromFileTTF( XS( "C:\\Windows\\Fonts\\arial.ttf" ), font_cfg.SizePixels, &font_cfg, glyph_ranges );
...
ImFont* ImFontAtlas::AddFontFromFileTTF( const char* filename, float size_pixels, const ImFontConfig* font_cfg_template, const ImWchar* glyph_ranges )
{
	IM_ASSERT( !Locked && XS( "Cannot modify a locked ImFontAtlas between NewFrame() and EndFrame/Render()!" ) );
	size_t data_size = 0;
	void* data = ImFileLoadToMemory( filename, XS( "rb" ), &data_size, 0 );
	if ( !data )
	{
		IM_ASSERT_USER_ERROR( 0, XS( "Could not load font file!" ) );
		return NULL;
	}
	ImFontConfig font_cfg = font_cfg_template ? *font_cfg_template : ImFontConfig();
	if ( font_cfg.Name[ 0 ] == '\0' )
	{
		...
		ImFormatString( font_cfg.Name, IM_ARRAYSIZE( font_cfg.Name ), XS( "%s, %.0fpx" ), p, size_pixels );
	}
...
bool    ImFontAtlasBuildWithStbTruetype( ImFontAtlas* atlas )
{
	...
	for ( int src_i = 0; src_i < atlas->ConfigData.Size; src_i++ )
	{
		...
		IM_ASSERT( font_offset >= 0 && XS( "FontData is incorrect, or FontNo cannot be found." ) );
