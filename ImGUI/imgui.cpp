...
{
	ImGuiContext& g = *GImGui;
	return g.IO.GetClipboardTextFn ? g.IO.GetClipboardTextFn( g.IO.ClipboardUserData ) : XS( "" );
}
...
{
	bool error = false;
	if ( strcmp( version, IMGUI_VERSION ) != 0 ) { error = true; IM_ASSERT( strcmp( version, IMGUI_VERSION ) == 0 && XS( "Mismatched version string!" ) ); }
	if ( sz_io != sizeof( ImGuiIO ) ) { error = true; IM_ASSERT( sz_io == sizeof( ImGuiIO ) && XS( "Mismatched struct layout!" ) ); }
	if ( sz_style != sizeof( ImGuiStyle ) ) { error = true; IM_ASSERT( sz_style == sizeof( ImGuiStyle ) && XS( "Mismatched struct layout!" ) ); }
	if ( sz_vec2 != sizeof( ImVec2 ) ) { error = true; IM_ASSERT( sz_vec2 == sizeof( ImVec2 ) && XS( "Mismatched struct layout!" ) ); }
	if ( sz_vec4 != sizeof( ImVec4 ) ) { error = true; IM_ASSERT( sz_vec4 == sizeof( ImVec4 ) && XS( "Mismatched struct layout!" ) ); }
	if ( sz_vert != sizeof( ImDrawVert ) ) { error = true; IM_ASSERT( sz_vert == sizeof( ImDrawVert ) && XS( "Mismatched struct layout!" ) ); }
	if ( sz_idx != sizeof( ImDrawIdx ) ) { error = true; IM_ASSERT( sz_idx == sizeof( ImDrawIdx ) && XS( "Mismatched struct layout!" ) ); }
	return !error;
}
...
ImGuiIO& ImGui::GetIO()
{
	IM_ASSERT( GImGui != NULL && XS( "No current context. Did you call ImGui::CreateContext() and ImGui::SetCurrentContext() ?" ) );
	return GImGui->IO;
}
...
static void NewFrameSanityChecks()
{
	...
	IM_ASSERT( ( g.IO.DeltaTime > 0.0f || g.FrameCount == 0 ) && XS( "Need a positive DeltaTime!" ) );
	IM_ASSERT( ( g.FrameCount == 0 || g.FrameCountEnded == g.FrameCount ) && XS( "Forgot to call Render() or EndFrame() at the end of the previous frame?" ) );
	IM_ASSERT( g.IO.DisplaySize.x >= 0.0f && g.IO.DisplaySize.y >= 0.0f && XS( "Invalid DisplaySize value!" ) );
	IM_ASSERT( g.IO.Fonts->Fonts.Size > 0 && XS( "Font Atlas not built. Did you call io.Fonts->GetTexDataAsRGBA32() / GetTexDataAsAlpha8() ?" ) );
	IM_ASSERT( g.IO.Fonts->Fonts[ 0 ]->IsLoaded() && XS( "Font Atlas not built. Did you call io.Fonts->GetTexDataAsRGBA32() / GetTexDataAsAlpha8() ?" ) );
	IM_ASSERT( g.Style.CurveTessellationTol > 0.0f && XS( "Invalid style setting!" ) );
	IM_ASSERT( g.Style.CircleSegmentMaxError > 0.0f && XS( "Invalid style setting!" ) );
	IM_ASSERT( g.Style.Alpha >= 0.0f && g.Style.Alpha <= 1.0f && XS( "Invalid style setting. Alpha cannot be negative (allows us to avoid a few clamps in color computations)!" ) );
	IM_ASSERT( g.Style.WindowMinSize.x >= 1.0f && g.Style.WindowMinSize.y >= 1.0f && XS( "Invalid style setting." ) );
	IM_ASSERT( g.Style.WindowMenuButtonPosition == ImGuiDir_None || g.Style.WindowMenuButtonPosition == ImGuiDir_Left || g.Style.WindowMenuButtonPosition == ImGuiDir_Right );
	for ( int n = 0; n < ImGuiKey_COUNT; n++ )
		IM_ASSERT( g.IO.KeyMap[ n ] >= -1 && g.IO.KeyMap[ n ] < IM_ARRAYSIZE( g.IO.KeysDown ) && XS( "io.KeyMap[] contains an out of bound value (need to be 0..512, or -1 for unmapped key)" ) );

	// Perform simple check: required key mapping (we intentionally do NOT check all keys to not pressure user into setting up everything, but Space is required and was only recently added in 1.60 WIP)
	if ( g.IO.ConfigFlags & ImGuiConfigFlags_NavEnableKeyboard )
		IM_ASSERT( g.IO.KeyMap[ ImGuiKey_Space ] != -1 && XS( "ImGuiKey_Space is not mapped, required for keyboard navigation." ) );
...
void ImGui::UpdateDebugToolItemPicker()
{
	...
	if ( g.DebugItemPickerActive )
	{
		...
		ImGui::BeginTooltip();
		ImGui::Text( XS( "HoveredId: 0x%08X" ), hovered_id );
		ImGui::Text( XS( "Press ESC to abort picking." ) );
		ImGui::TextColored( GetStyleColorVec4( hovered_id ? ImGuiCol_Text : ImGuiCol_TextDisabled ), XS( "Click to break in debugger!" ) );
		ImGui::EndTooltip();
	}
}
...
void ImGui::Initialize( ImGuiContext* context )
{
	...
	// Add .ini handle for ImGuiWindow type
	{
		...
		ini_handler.TypeName = XS( "Window" );
		ini_handler.TypeHash = ImHashStr( XS( "Window" ) );
	...
	{
		ImGuiSettingsHandler ini_handler;
		ini_handler.TypeName = XS( "Table" );
		ini_handler.TypeHash = ImHashStr( XS( "Table" ) );
...
static void AddDrawListToDrawData( ImVector<ImDrawList*>* out_list, ImDrawList* draw_list )
{
	...
	if ( sizeof( ImDrawIdx ) == 2 )
		IM_ASSERT( draw_list->_VtxCurrentIdx < ( 1 << 16 ) && XS( "Too many vertices in ImDrawList using 16-bit indices. Read comment above" ) );
...
void ImGui::EndFrame()
{
	...
	if ( g.FrameCountEnded == g.FrameCount )          // Don't process EndFrame() multiple times.
		return;
	IM_ASSERT( g.WithinFrameScope && XS( "Forgot to call ImGui::NewFrame()?" ) );
...
bool ImGui::BeginChildEx( const char* name, ImGuiID id, const ImVec2& size_arg, bool border, ImGuiWindowFlags flags )
{
	...
	char title[ 256 ];
	if ( name )
		ImFormatString( title, IM_ARRAYSIZE( title ), XS( "%s/%s_%08X" ), parent_window->Name, name, id );
	else
		ImFormatString( title, IM_ARRAYSIZE( title ), XS( "%s/%08X" ), parent_window->Name, id );
...
void ImGui::RenderWindowTitleBarContents( ImGuiWindow* window, const ImRect& title_bar_rect, const char* name, bool* p_open )
{
	...
	const char* UNSAVED_DOCUMENT_MARKER = XS( "*" );
...
void ImGui::End()
{
	...
	{
		IM_ASSERT_USER_ERROR( g.CurrentWindowStack.Size > 1, XS( "Calling End() too many times!" ) );
		return;
	}
	...
	if ( window->Flags & ImGuiWindowFlags_ChildWindow )
		IM_ASSERT_USER_ERROR( g.WithinEndChild, XS( "Must call EndChild() and not End()!" ) );
...
void ImGui::PushStyleVar( ImGuiStyleVar idx, float val )
{
	...
	IM_ASSERT( 0 && XS( "Called PushStyleVar() float variant but variable is not a float!" ) );
}
...
void ImGui::PushStyleVar( ImGuiStyleVar idx, const ImVec2& val )
{
	...
	IM_ASSERT( 0 && XS( "Called PushStyleVar() ImVec2 variant but variable is not a ImVec2!" ) );
}
...
const char* ImGui::GetStyleColorName( ImGuiCol idx )
{
	...
	switch ( idx )
	{
	case ImGuiCol_Text: return XS( "Text" );
	case ImGuiCol_TextDisabled: return XS( "TextDisabled" );
	case ImGuiCol_WindowBg: return XS( "WindowBg" );
	case ImGuiCol_ChildBg: return XS( "ChildBg" );
	case ImGuiCol_PopupBg: return XS( "PopupBg" );
	case ImGuiCol_Border: return XS( "Border" );
	case ImGuiCol_BorderShadow: return XS( "BorderShadow" );
	case ImGuiCol_FrameBg: return XS( "FrameBg" );
	case ImGuiCol_FrameBgHovered: return XS( "FrameBgHovered" );
	case ImGuiCol_FrameBgActive: return XS( "FrameBgActive" );
	case ImGuiCol_TitleBg: return XS( "TitleBg" );
	case ImGuiCol_TitleBgActive: return XS( "TitleBgActive" );
	case ImGuiCol_TitleBgCollapsed: return XS( "TitleBgCollapsed" );
	case ImGuiCol_MenuBarBg: return XS( "MenuBarBg" );
	case ImGuiCol_ScrollbarBg: return XS( "ScrollbarBg" );
	case ImGuiCol_ScrollbarGrab: return XS( "ScrollbarGrab" );
	case ImGuiCol_ScrollbarGrabHovered: return XS( "ScrollbarGrabHovered" );
	case ImGuiCol_ScrollbarGrabActive: return XS( "ScrollbarGrabActive" );
	case ImGuiCol_CheckMark: return XS( "CheckMark" );
	case ImGuiCol_SliderGrab: return XS( "SliderGrab" );
	case ImGuiCol_SliderGrabActive: return XS( "SliderGrabActive" );
	case ImGuiCol_Button: return XS( "Button" );
	case ImGuiCol_ButtonHovered: return XS( "ButtonHovered" );
	case ImGuiCol_ButtonActive: return XS( "ButtonActive" );
	case ImGuiCol_Header: return XS( "Header" );
	case ImGuiCol_HeaderHovered: return XS( "HeaderHovered" );
	case ImGuiCol_HeaderActive: return XS( "HeaderActive" );
	case ImGuiCol_Separator: return XS( "Separator" );
	case ImGuiCol_SeparatorHovered: return XS( "SeparatorHovered" );
	case ImGuiCol_SeparatorActive: return XS( "SeparatorActive" );
	case ImGuiCol_ResizeGrip: return XS( "ResizeGrip" );
	case ImGuiCol_ResizeGripHovered: return XS( "ResizeGripHovered" );
	case ImGuiCol_ResizeGripActive: return XS( "ResizeGripActive" );
	case ImGuiCol_Tab: return XS( "Tab" );
	case ImGuiCol_TabHovered: return XS( "TabHovered" );
	case ImGuiCol_TabActive: return XS( "TabActive" );
	case ImGuiCol_TabUnfocused: return XS( "TabUnfocused" );
	case ImGuiCol_TabUnfocusedActive: return XS( "TabUnfocusedActive" );
	case ImGuiCol_PlotLines: return XS( "PlotLines" );
	case ImGuiCol_PlotLinesHovered: return XS( "PlotLinesHovered" );
	case ImGuiCol_PlotHistogram: return XS( "PlotHistogram" );
	case ImGuiCol_PlotHistogramHovered: return XS( "PlotHistogramHovered" );
	case ImGuiCol_TextSelectedBg: return XS( "TextSelectedBg" );
	case ImGuiCol_DragDropTarget: return XS( "DragDropTarget" );
	case ImGuiCol_NavHighlight: return XS( "NavHighlight" );
	case ImGuiCol_NavWindowingHighlight: return XS( "NavWindowingHighlight" );
	case ImGuiCol_NavWindowingDimBg: return XS( "NavWindowingDimBg" );
	case ImGuiCol_ModalWindowDimBg: return XS( "ModalWindowDimBg" );
	}
	IM_ASSERT( 0 );
	return XS( "Unknown" );
}
...
static void ImGui::ErrorCheckEndFrame()
{
	...
	if ( g.CurrentWindowStack.Size != 1 )
	{
		if ( g.CurrentWindowStack.Size > 1 )
		{
			IM_ASSERT_USER_ERROR( g.CurrentWindowStack.Size == 1, XS( "Mismatched Begin/BeginChild vs End/EndChild calls: did you forget to call End/EndChild?" ) );
			while ( g.CurrentWindowStack.Size > 1 )
				End();
		}
		else
		{
			IM_ASSERT_USER_ERROR( g.CurrentWindowStack.Size == 1, XS( "Mismatched Begin/BeginChild vs End/EndChild calls: did you call End/EndChild too much?" ) );
		}
	}
...
void ImGui::BeginTooltipEx( ImGuiWindowFlags extra_flags, ImGuiTooltipFlags tooltip_flags )
{
	...
	char window_name[ 16 ];
	ImFormatString( window_name, IM_ARRAYSIZE( window_name ), XS( "##Tooltip_%02d" ), g.TooltipOverrideCount );
...
bool ImGui::BeginPopupEx( ImGuiID id, ImGuiWindowFlags flags )
{
	...
	if ( flags & ImGuiWindowFlags_ChildMenu )
		ImFormatString( name, IM_ARRAYSIZE( name ), XS( "##Menu_%02d" ), g.BeginPopupStack.Size ); // Recycle windows based on depth
	else
		ImFormatString( name, IM_ARRAYSIZE( name ), XS( "##Popup_%08x" ), id ); // Not recycling, so we can close/open during the same frame
...
static bool ImGui::NavScoreItem( ImGuiNavMoveResult* result, ImRect cand )
{
	...
	{
		ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "dbox (%.2f,%.2f->%.4f)\ndcen (%.2f,%.2f->%.4f)\nd (%.2f,%.2f->%.4f)\nnav %c, quadrant %c" ), dbx, dby, dist_box, dcx, dcy, dist_center, dax, day, dist_axial, XS( XS( "WENS" ) )[ g.NavMoveDir ], XS( XS( "WENS" ) )[ quadrant ] );
	...
	else if ( g.IO.KeyCtrl ) // Hold to preview score in matching quadrant. Press C to rotate.
	{
		...
		{
			ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "%.0f/%.0f" ), dist_box, dist_center );
			ImDrawList* draw_list = GetForegroundDrawList( window );
...
#if 0
	if ( g.NavScoringCount > 0 ) IMGUI_DEBUG_LOG( XS( "NavScoringCount %d for '%s' layer %d (Init:%d, Move:%d)\n" ), g.FrameCount, g.NavScoringCount, g.NavWindow ? g.NavWindow->Name : XS( "NULL" ), g.NavLayer, g.NavInitRequest || g.NavInitResultId != 0, g.NavMoveRequest );
#endif
...
#undef NAV_MAP_KEY
	...
	if ( g.NavWindow )
	{
		...
		if ( 1 ) { ImU32 col = ( !g.NavWindow->Hidden ) ? IM_COL32( 255, 0, 255, 255 ) : IM_COL32( 255, 0, 0, 255 ); ImVec2 p = NavCalcPreferredRefPos(); char buf[ 32 ]; ImFormatString( buf, 32, XS( "%d" ), g.NavLayer ); draw_list->AddCircleFilled( p, 3.0f, col ); draw_list->AddText( NULL, 13.0f, p + ImVec2( 8, -4 ), col, buf ); }
	}
#endif
}
...
static const char* GetFallbackWindowNameForWindowingList( ImGuiWindow* window )
{
	if ( window->Flags & ImGuiWindowFlags_Popup )
		return XS( "(Popup)" );
	if ( ( window->Flags & ImGuiWindowFlags_MenuBar ) && strcmp( window->Name, XS( "##MainMenuBar" ) ) == 0 )
		return XS( "(Main menu bar)" );
	return XS( "(Untitled)" );
}
...
void ImGui::NavUpdateWindowingOverlay()
{
	...
	if ( g.NavWindowingList == NULL )
		g.NavWindowingList = FindWindowByName( XS( "###NavWindowingList" ) );
	SetNextWindowSizeConstraints( ImVec2( g.IO.DisplaySize.x * 0.20f, g.IO.DisplaySize.y * 0.20f ), ImVec2( FLT_MAX, FLT_MAX ) );
	SetNextWindowPos( g.IO.DisplaySize * 0.5f, ImGuiCond_Always, ImVec2( 0.5f, 0.5f ) );
	PushStyleVar( ImGuiStyleVar_WindowPadding, g.Style.WindowPadding * 2.0f );
	Begin( XS( "###NavWindowingList" ), NULL, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoFocusOnAppearing | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoInputs | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoSavedSettings );
...
bool ImGui::BeginDragDropSource( ImGuiDragDropFlags flags )
{
	...
	{
		window = NULL;
		source_id = ImHashStr( XS( "#SourceExtern" ) );
...
void ImGui::EndDragDropSource()
{
	...
	IM_ASSERT( g.DragDropWithinSourceOrTarget && XS( "Not after a BeginDragDropSource()?" ) );
...
bool ImGui::SetDragDropPayload( const char* type, const void* data, size_t data_size, ImGuiCond cond )
{
	...
	IM_ASSERT( type != NULL );
	IM_ASSERT( strlen( type ) < IM_ARRAYSIZE( payload.DataType ) && XS( "Payload type can be at most 32 characters long" ) );
	...
	for ( ;;)
	{
		...
		if ( !is_last_line || ( line_start != line_end ) )
		{
			...
			if ( log_new_line || !is_first_line )
				LogText( XS( "\r\n %*s%.*s" ), tree_depth * 4, XS( "" ), char_count, line_start );
			else if ( g.LogLineFirstItem )
				LogText( XS( "%*s%.*s" ), tree_depth * 4, XS( "" ), char_count, line_start );
			else
				LogText( XS( " %.*s" ), char_count, line_start );
...
void ImGui::LogToFile( int auto_open_depth, const char* filename )
{
	...
	if ( !filename || !filename[ 0 ] )
		return;
	ImFileHandle f = ImFileOpen( filename, XS( "ab" ) );
	if ( !f )
	{
		IM_ASSERT( 0 );
		return;
...
void ImGui::LogButtons()
{
	...
	PushID( XS( "LogButtons" ) );
#ifndef IMGUI_DISABLE_TTY_FUNCTIONS
	const bool log_to_tty = Button( XS( "Log To TTY" ) ); SameLine();
#else
	const bool log_to_tty = false;
#endif
	const bool log_to_file = Button( XS( "Log To File" ) ); SameLine();
	const bool log_to_clipboard = Button( XS( "Log To Clipboard" ) ); SameLine();
	PushAllowKeyboardFocus( false );
	SetNextItemWidth( 80.0f );
	SliderInt( XS( "Default Depth" ), &g.LogDepthToExpandDefault, 0, 9, NULL );
...
void ImGui::LoadIniSettingsFromDisk( const char* ini_filename )
{
	...
	char* file_data = ( char* )ImFileLoadToMemory( ini_filename, XS( "rb" ), &file_data_size );
	if ( !file_data )
		return;
...
void ImGui::SaveIniSettingsToDisk( const char* ini_filename )
{
	...
	ImFileHandle f = ImFileOpen( ini_filename, XS( "wt" ) );
	if ( !f )
		return;
...
static void WindowSettingsHandler_ReadLine( ImGuiContext*, ImGuiSettingsHandler*, void* entry, const char* line )
{
	...
	int x, y;
	int i;
	if ( sscanf( line, XS( "Pos=%i,%i" ), &x, &y ) == 2 )         settings->Pos = ImVec2ih( ( short )x, ( short )y );
	else if ( sscanf( line, XS( "Size=%i,%i" ), &x, &y ) == 2 )   settings->Size = ImVec2ih( ( short )x, ( short )y );
	else if ( sscanf( line, XS( "Collapsed=%d" ), &i ) == 1 )     settings->Collapsed = ( i != 0 );
}
...
static void WindowSettingsHandler_WriteAll( ImGuiContext* ctx, ImGuiSettingsHandler* handler, ImGuiTextBuffer* buf )
{
	...
	for ( ImGuiWindowSettings* settings = g.SettingsWindows.begin(); settings != NULL; settings = g.SettingsWindows.next_chunk( settings ) )
	{
		...
		buf->appendf( XS( "Pos=%d,%d\n" ), settings->Pos.x, settings->Pos.y );
		buf->appendf( XS( "Size=%d,%d\n" ), settings->Size.x, settings->Size.y );
		buf->appendf( XS( "Collapsed=%d\n" ), settings->Collapsed );
		buf->append( XS( "\n" ) );
	}
}
...
static void SetClipboardTextFn_DefaultImpl( void*, const char* text )
{
	...
	if ( cf_data )
	{
		PasteboardPutItemFlavor( main_clipboard, ( PasteboardItemID )1, CFSTR( XS( "public.utf8-plain-text" ) ), cf_data, 0 );
		CFRelease( cf_data );
	}
}
...
static const char* GetClipboardTextFn_DefaultImpl( void* )
{
	...
	for ( ItemCount i = 0; i < item_count; i++ )
	{
		...
		{
			CFDataRef cf_data;
			if ( PasteboardCopyItemFlavorData( main_clipboard, item_id, CFSTR( XS( "public.utf8-plain-text" ) ), &cf_data ) == noErr )
			{
...
{
	if ( !ImGui::Begin( XS( "Dear ImGui Metrics" ), p_open ) )
	{
		ImGui::End();
		return;
	}
	...
	const char* wrt_rects_names[ WRT_Count ] = { XS( "OuterRect" ), XS( "OuterRectClipped" ), XS( "InnerRect" ), XS( "InnerClipRect" ), XS( "WorkRect" ), XS( "Content" ), XS( "ContentRegionRect" ) };
	...
	const char* trt_rects_names[ TRT_Count ] = { XS( "OuterRect" ), XS( "WorkRect" ), XS( "HostClipRect" ), XS( "InnerClipRect" ), XS( "BackgroundClipRect" ), XS( "ColumnsRect" ), XS( "ColumnsClipRect" ), XS( "ColumnsContentHeadersUsed" ), XS( "ColumnsContentHeadersDesired" ), XS( "ColumnsContentRowsFrozen" ), XS( "ColumnsContentRowsUnfrozen" ) };
	...
	ImGuiContext& g = *GImGui;
	ImGuiIO& io = ImGui::GetIO();
	ImGui::Text( XS( "Dear ImGui %s" ), ImGui::GetVersion() );
	ImGui::Text( XS( "Application average %.3f ms/frame (%.1f FPS)" ), 1000.0f / io.Framerate, io.Framerate );
	ImGui::Text( XS( "%d vertices, %d indices (%d triangles)" ), io.MetricsRenderVertices, io.MetricsRenderIndices, io.MetricsRenderIndices / 3 );
	ImGui::Text( XS( "%d active windows (%d visible)" ), io.MetricsActiveWindows, io.MetricsRenderWindows );
	ImGui::Text( XS( "%d active allocations" ), io.MetricsActiveAllocations );
	ImGui::Separator();
	...
	struct Funcs
	{
		...
		{
			bool node_open = ImGui::TreeNode( draw_list, XS( "%s: '%s' %d vtx, %d indices, %d cmds" ), label, draw_list->_OwnerName ? draw_list->_OwnerName : XS( "" ), draw_list->VtxBuffer.Size, draw_list->IdxBuffer.Size, draw_list->CmdBuffer.Size );
			...
			if ( window && !window->WasActive )
				ImGui::TextDisabled( XS( "Warning: owning Window is inactive. This DrawList is not being rendered!" ) );
			...
			for ( const ImDrawCmd* pcmd = draw_list->CmdBuffer.begin(); pcmd < draw_list->CmdBuffer.end(); elem_offset += pcmd->ElemCount, pcmd++ )
			{
				...
				if ( pcmd->UserCallback )
				{
					ImGui::BulletText( XS( "Callback %p, user_data %p" ), pcmd->UserCallback, pcmd->UserCallbackData );
					continue;
				}

				ImDrawIdx* idx_buffer = ( draw_list->IdxBuffer.Size > 0 ) ? draw_list->IdxBuffer.Data : NULL;
				char buf[ 300 ];
				ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "DrawCmd: %4d triangles, Tex 0x%p, ClipRect (%4.0f,%4.0f)-(%4.0f,%4.0f)" ),
					pcmd->ElemCount / 3, ( void* )( intptr_t )pcmd->TextureId,
					pcmd->ClipRect.x, pcmd->ClipRect.y, pcmd->ClipRect.z, pcmd->ClipRect.w );
				bool pcmd_node_open = ImGui::TreeNode( ( void* )( pcmd - draw_list->CmdBuffer.begin() ), XS( "%s" ), buf );
				...
				ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "Mesh: ElemCount: %d, VtxOffset: +%d, IdxOffset: +%d, Area: ~%0.f px" ), pcmd->ElemCount, pcmd->VtxOffset, pcmd->IdxOffset, total_area );
				...
				while ( clipper.Step() )
					for ( int prim = clipper.DisplayStart, idx_i = elem_offset + clipper.DisplayStart * 3; prim < clipper.DisplayEnd; prim++ )
					{
						...
						for ( int n = 0; n < 3; n++, idx_i++ )
						{
							...
							triangle[ n ] = v.pos;
							buf_p += ImFormatString( buf_p, buf_end - buf_p, XS( "%s %04d: pos (%8.2f,%8.2f), uv (%.6f,%.6f), col %08X\n" ),
								( n == 0 ) ? XS( "Vert:" ) : XS( "     " ), idx_i, v.pos.x, v.pos.y, v.uv.x, v.uv.y, v.col );
						}
		...
		{
			if ( !ImGui::TreeNode( ( void* )( uintptr_t )columns->ID, XS( "Columns Id: 0x%08X, Count: %d, Flags: 0x%04X" ), columns->ID, columns->Count, columns->Flags ) )
				return;
			ImGui::BulletText( XS( "Width: %.1f (MinX: %.1f, MaxX: %.1f)" ), columns->OffMaxX - columns->OffMinX, columns->OffMinX, columns->OffMaxX );
			for ( int column_n = 0; column_n < columns->Columns.Size; column_n++ )
				ImGui::BulletText( XS( "Column %02d: OffsetNorm %.3f (= %.1f px)" ), column_n, columns->Columns[ column_n ].OffsetNorm, GetColumnOffsetFromNorm( columns, columns->Columns[ column_n ].OffsetNorm ) );
			ImGui::TreePop();
		}

		static void NodeWindows( ImVector<ImGuiWindow*>& windows, const char* label )
		{
			if ( !ImGui::TreeNode( label, XS( "%s (%d)" ), label, windows.Size ) )
				return;
			for ( int i = 0; i < windows.Size; i++ )
			{
				ImGui::PushID( windows[ i ] );
				Funcs::NodeWindow( windows[ i ], XS( "Window" ) );
				ImGui::PopID();
			}
			ImGui::TreePop();
		}
		...
		static void NodeWindow( ImGuiWindow* window, const char* label )
		{
			if ( window == NULL )
			{
				ImGui::BulletText( XS( "%s: NULL" ), label );
				return;
			}
			bool open = ImGui::TreeNode( label, XS( "%s '%s', %d @ 0x%p" ), label, window->Name, ( window->Active || window->WasActive ), window );
			...
			if ( !open )
				return;
			ImGuiWindowFlags flags = window->Flags;
			NodeDrawList( window, window->DrawList, XS( "DrawList" ) );
			ImGui::BulletText( XS( "Pos: (%.1f,%.1f), Size: (%.1f,%.1f), ContentSize (%.1f,%.1f)" ), window->Pos.x, window->Pos.y, window->Size.x, window->Size.y, window->ContentSize.x, window->ContentSize.y );
			ImGui::BulletText( XS( "Flags: 0x%08X (%s%s%s%s%s%s%s%s%s..)" ), flags,
				( flags & ImGuiWindowFlags_ChildWindow ) ? XS( "Child " ) : XS( XS( XS( "" ) ) ), ( flags & ImGuiWindowFlags_Tooltip ) ? XS( "Tooltip " ) : XS( XS( XS( "" ) ) ), ( flags & ImGuiWindowFlags_Popup ) ? XS( "Popup " ) : XS( XS( XS( "" ) ) ),
				( flags & ImGuiWindowFlags_Modal ) ? XS( "Modal " ) : XS( XS( XS( "" ) ) ), ( flags & ImGuiWindowFlags_ChildMenu ) ? XS( "ChildMenu " ) : XS( XS( XS( "" ) ) ), ( flags & ImGuiWindowFlags_NoSavedSettings ) ? XS( "NoSavedSettings " ) : XS( XS( XS( "" ) ) ),
				( flags & ImGuiWindowFlags_NoMouseInputs ) ? XS( "NoMouseInputs" ) : XS( XS( XS( "" ) ) ), ( flags & ImGuiWindowFlags_NoNavInputs ) ? XS( "NoNavInputs" ) : XS( XS( XS( "" ) ) ), ( flags & ImGuiWindowFlags_AlwaysAutoResize ) ? XS( "AlwaysAutoResize" ) : XS( XS( XS( "" ) ) ) );
			ImGui::BulletText( XS( "Scroll: (%.2f/%.2f,%.2f/%.2f) Scrollbar:%s%s" ), window->Scroll.x, window->ScrollMax.x, window->Scroll.y, window->ScrollMax.y, window->ScrollbarX ? XS( "X" ) : XS( XS( "" ) ), window->ScrollbarY ? XS( "Y" ) : XS( XS( "" ) ) );
			ImGui::BulletText( XS( "Active: %d/%d, WriteAccessed: %d, BeginOrderWithinContext: %d" ), window->Active, window->WasActive, window->WriteAccessed, ( window->Active || window->WasActive ) ? window->BeginOrderWithinContext : -1 );
			ImGui::BulletText( XS( "Appearing: %d, Hidden: %d (CanSkip %d Cannot %d), SkipItems: %d" ), window->Appearing, window->Hidden, window->HiddenFramesCanSkipItems, window->HiddenFramesCannotSkipItems, window->SkipItems );
			ImGui::BulletText( XS( "NavLastIds: 0x%08X,0x%08X, NavLayerActiveMask: %X" ), window->NavLastIds[ 0 ], window->NavLastIds[ 1 ], window->DC.NavLayerActiveMask );
			ImGui::BulletText( XS( "NavLastChildNavWindow: %s" ), window->NavLastChildNavWindow ? window->NavLastChildNavWindow->Name : XS( "NULL" ) );
			if ( !window->NavRectRel[ 0 ].IsInverted() )
				ImGui::BulletText( XS( "NavRectRel[0]: (%.1f,%.1f)(%.1f,%.1f)" ), window->NavRectRel[ 0 ].Min.x, window->NavRectRel[ 0 ].Min.y, window->NavRectRel[ 0 ].Max.x, window->NavRectRel[ 0 ].Max.y );
			else
				ImGui::BulletText( XS( "NavRectRel[0]: <None>" ) );
			if ( window->RootWindow != window ) NodeWindow( window->RootWindow, XS( "RootWindow" ) );
			if ( window->ParentWindow != NULL ) NodeWindow( window->ParentWindow, XS( "ParentWindow" ) );
			if ( window->DC.ChildWindows.Size > 0 ) NodeWindows( window->DC.ChildWindows, XS( "ChildWindows" ) );
			if ( window->ColumnsStorage.Size > 0 && ImGui::TreeNode( XS( "Columns" ), XS( "Columns sets (%d)" ), window->ColumnsStorage.Size ) )
			{
				for ( int n = 0; n < window->ColumnsStorage.Size; n++ )
					NodeColumns( &window->ColumnsStorage[ n ] );
				ImGui::TreePop();
			}
			NodeStorage( &window->StateStorage, XS( "Storage" ) );
			ImGui::TreePop();
		}
		...
		static void NodeTabBar( ImGuiTabBar* tab_bar )
		{
			...
			p += ImFormatString( p, buf_end - p, XS( "TabBar (%d tabs)%s" ), tab_bar->Tabs.Size, ( tab_bar->PrevFrameVisible < ImGui::GetFrameCount() - 2 ) ? XS( " *Inactive*" ) : XS( "" ) );
			if ( ImGui::TreeNode( tab_bar, XS( "%s" ), buf ) )
			{
				for ( int tab_n = 0; tab_n < tab_bar->Tabs.Size; tab_n++ )
				{
					const ImGuiTabItem* tab = &tab_bar->Tabs[ tab_n ];
					ImGui::PushID( tab );
					if ( ImGui::SmallButton( XS( "<" ) ) ) { TabBarQueueChangeTabOrder( tab_bar, tab, -1 ); } ImGui::SameLine( 0, 2 );
					if ( ImGui::SmallButton( XS( ">" ) ) ) { TabBarQueueChangeTabOrder( tab_bar, tab, +1 ); } ImGui::SameLine();
					ImGui::Text( XS( "%02d%c Tab 0x%08X '%s'" ), tab_n, ( tab->ID == tab_bar->SelectedTabId ) ? '*' : ' ', tab->ID, ( tab->NameOffset != -1 ) ? tab_bar->GetTabName( tab ) : XS( "" ) );
					ImGui::PopID();
				}
				ImGui::TreePop();
			}
		...
		{
			if ( !ImGui::TreeNode( label, XS( "%s: %d entries, %d bytes" ), label, storage->Data.Size, storage->Data.size_in_bytes() ) )
				return;
	...
	Funcs::NodeWindows( g.Windows, XS( "Windows" ) );
	//Funcs::NodeWindows(g.WindowsFocusOrder, "WindowsFocusOrder");
	if ( ImGui::TreeNode( XS( "DrawLists" ), XS( "Active DrawLists (%d)" ), g.DrawDataBuilder.Layers[ 0 ].Size ) )
	{
		for ( int i = 0; i < g.DrawDataBuilder.Layers[ 0 ].Size; i++ )
			Funcs::NodeDrawList( NULL, g.DrawDataBuilder.Layers[ 0 ][ i ], XS( "DrawList" ) );
		ImGui::TreePop();
	}

	// Details for Popups
	if ( ImGui::TreeNode( XS( "Popups" ), XS( "Popups (%d)" ), g.OpenPopupStack.Size ) )
	{
		for ( int i = 0; i < g.OpenPopupStack.Size; i++ )
		{
			ImGuiWindow* window = g.OpenPopupStack[ i ].Window;
			ImGui::BulletText( XS( "PopupID: %08x, Window: '%s'%s%s" ), g.OpenPopupStack[ i ].PopupId, window ? window->Name : XS( "NULL" ), window && ( window->Flags & ImGuiWindowFlags_ChildWindow ) ? XS( " ChildWindow" ) : XS( XS( "" ) ), window && ( window->Flags & ImGuiWindowFlags_ChildMenu ) ? XS( " ChildMenu" ) : XS( XS( "" ) ) );
		}
		ImGui::TreePop();
	}

	// Details for TabBars
	if ( ImGui::TreeNode( XS( "TabBars" ), XS( "Tab Bars (%d)" ), g.TabBars.GetSize() ) )
	{
...
#ifdef IMGUI_HAS_TABLE
	if ( ImGui::TreeNode( XS( "Tables" ), XS( "Tables (%d)" ), g.Tables.GetSize() ) )
	{
	...
	// Misc Details
	if ( ImGui::TreeNode( XS( "Internal state" ) ) )
	{
		const char* input_source_names[] = { XS( "None" ), XS( "Mouse" ), XS( "Nav" ), XS( "NavKeyboard" ), XS( "NavGamepad" ) }; IM_ASSERT( IM_ARRAYSIZE( input_source_names ) == ImGuiInputSource_COUNT );
		ImGui::Text( XS( "HoveredWindow: '%s'" ), g.HoveredWindow ? g.HoveredWindow->Name : XS( "NULL" ) );
		ImGui::Text( XS( "HoveredRootWindow: '%s'" ), g.HoveredRootWindow ? g.HoveredRootWindow->Name : XS( "NULL" ) );
		ImGui::Text( "HoveredId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d", g.HoveredId, g.HoveredIdPreviousFrame, g.HoveredIdTimer, g.HoveredIdAllowOverlap ); // Data is "in-flight" so depending on when the Metrics window is called we may see current frame information or not
		ImGui::Text( XS( "ActiveId: 0x%08X/0x%08X (%.2f sec), AllowOverlap: %d, Source: %s" ), g.ActiveId, g.ActiveIdPreviousFrame, g.ActiveIdTimer, g.ActiveIdAllowOverlap, input_source_names[ g.ActiveIdSource ] );
		ImGui::Text( XS( "ActiveIdWindow: '%s'" ), g.ActiveIdWindow ? g.ActiveIdWindow->Name : XS( "NULL" ) );
		ImGui::Text( XS( "MovingWindow: '%s'" ), g.MovingWindow ? g.MovingWindow->Name : XS( "NULL" ) );
		ImGui::Text( XS( "NavWindow: '%s'" ), g.NavWindow ? g.NavWindow->Name : XS( "NULL" ) );
		ImGui::Text( XS( "NavId: 0x%08X, NavLayer: %d" ), g.NavId, g.NavLayer );
		ImGui::Text( XS( "NavInputSource: %s" ), input_source_names[ g.NavInputSource ] );
		ImGui::Text( XS( "NavActive: %d, NavVisible: %d" ), g.IO.NavActive, g.IO.NavVisible );
		ImGui::Text( XS( "NavActivateId: 0x%08X, NavInputId: 0x%08X" ), g.NavActivateId, g.NavInputId );
		ImGui::Text( XS( "NavDisableHighlight: %d, NavDisableMouseHover: %d" ), g.NavDisableHighlight, g.NavDisableMouseHover );
		ImGui::Text( XS( "NavWindowingTarget: '%s'" ), g.NavWindowingTarget ? g.NavWindowingTarget->Name : XS( "NULL" ) );
		ImGui::Text( XS( "DragDrop: %d, SourceId = 0x%08X, Payload \" )%s\XS( " (%d bytes)" ), g.DragDropActive, g.DragDropPayload.SourceId, g.DragDropPayload.DataType, g.DragDropPayload.DataSize );
		ImGui::TreePop();
	}

	// Tools
	if ( ImGui::TreeNode( XS( "Tools" ) ) )
	{
		// The Item Picker tool is super useful to visually select an item and break into the call-stack of where it was submitted.
		if ( ImGui::Button( XS( "Item Picker.." ) ) )
			ImGui::DebugStartItemPicker();
		ImGui::SameLine();
		MetricsHelpMarker( XS( "Will call the IM_DEBUG_BREAK() macro to break in debugger.\nWarning: If you don't have a debugger attached, this will probably crash." ) );

		ImGui::Checkbox( XS( "Show windows begin order" ), &show_windows_begin_order );
		ImGui::Checkbox( XS( "Show windows rectangles" ), &show_windows_rects );
		ImGui::SameLine();
		...
		{
			ImGui::BulletText( XS( "'%s':" ), g.NavWindow->Name );
			ImGui::Indent();
			for ( int rect_n = 0; rect_n < WRT_Count; rect_n++ )
			{
				ImRect r = Funcs::GetWindowRect( g.NavWindow, rect_n );
				ImGui::Text( XS( "(%6.1f,%6.1f) (%6.1f,%6.1f) Size (%6.1f,%6.1f) %s" ), r.Min.x, r.Min.y, r.Max.x, r.Max.y, r.GetWidth(), r.GetHeight(), wrt_rects_names[ rect_n ] );
			}
			ImGui::Unindent();
		}
		ImGui::Checkbox( XS( "Show details when hovering ImDrawCmd node" ), &show_drawcmd_details );
		ImGui::TreePop();
	}
	...
	if ( show_windows_rects || show_windows_begin_order )
	{
		for ( int n = 0; n < g.Windows.Size; n++ )
		{
			...
			{
				char buf[ 32 ];
				ImFormatString( buf, IM_ARRAYSIZE( buf ), XS( "%d" ), window->BeginOrderWithinContext );
